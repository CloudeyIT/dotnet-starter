...
public class UserTests : IntegrationFixture
{
	...
	[Test]
	public async Task Cannot_Get_Me_As_Unauthenticated_User ()
	{
		var request = new GraphQLRequest
		{
			Query = @"
		...
		try
		{
			var response = await GraphQlClient.SendQueryAsync(request, () => new { me = new User() });
	...
	[Test]
	public async Task Cannot_Get_MyRoles_As_Unauthenticated_User ()
	{
		var request = new GraphQLRequest
		{
			Query = @"
